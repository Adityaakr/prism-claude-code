...
The commands are COPIED in, so your install drifts from the source as Prism ships new features.
To pull the latest and re-sync in one step:

```bash
# from inside your clone
git pull                       # or let the updater do it
bash scripts/prism-update.sh   # pull + re-copy into ~/.claude, then prove it with the drift check
```
