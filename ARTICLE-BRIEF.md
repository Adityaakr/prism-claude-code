...
**INSTRUCTION FOR THE ASSISTANT:** Using the source material below, write a polished
~1,500–2,000 word technical article about **Prism**. Requirements:
- Lead with a hook that explains the problem, then introduce Prism.
- Include a dedicated **Architecture** section and reproduce the ASCII architecture diagram
  (or render it as a Mermaid diagram).
- Cover: the problem, the philosophy, the eight commands, the deliberation engine, project
  memory, the enforcement hooks, a worked example, what makes it different, and honest limitations.
- Tone: technical but readable — for engineers evaluating AI agent tooling. Confident, not hypey.
- Do not invent features beyond what's described here. Keep the honest-limitations section intact.

---

## 1. One-line definition
**Prism is a multi-agent orchestration harness for Claude Code** — a set of slash commands that
turn a single hard question or task into a coordinated team of AI agents that fan out across
diverse perspectives, judge their disagreements, adversarially verify their conclusions, and loop
until the answer converges — while *remembering* the project, *grounding* every claim in real
code, and *enforcing* safety with hard hooks.

**The metaphor:** white light enters a prism and splits into a full spectrum. One question enters
Prism and splits into a spectrum of expert perspectives, then recombines into one verified answer.
...
## 3. Core philosophy (the principles)
...
### 5e. The lifecycle (architecture diagram)
```
              ┌─────────────────────────────────────────────┐
              │                                             │
...
## 7. What makes it different (the thesis)
...
## 8. Honest limitations (keep these — they make the article credible)
